...
const getPendingPrescriptions = (req, res) => {
    PrescriptionRequest.findAllPendingForPharmacy((error, results) => {
        if (error) {
            console.error('Error fetching pending prescriptions:', error);
            return res.status(500).json({ message: 'Server error' });
        }

        // No need for additional filtering since the query already filters for dispenser_role = 'pharmacy'
                ...
                DispensingRecord.create(dispensingData, (error, recordResults) => {
                    ...
                    db.execute(updateDrugStatusQuery, ['dispensed', prescription_drug_id], (error) => {
                        ...
                        db.execute(checkAllDrugsQuery, [prescription_id], (error, checkResults) => {
                            ...
                            console.log('total', total, 'dispensed_count', dispensed_count)
...
const getInventoryItem = (req, res) => {
    ...
    Inventory.findById(id, (err, inventoryItem) => {
        if (err) {
